...
class AboutMessagePassing < Neo::Koan
  ...
  def test_send_with_underscores_will_also_send_messages
    ...
    ## 答：有的类会定义 send 方法，从而原来的 send 被覆盖
  end
  ...
  def test_all_messages_are_caught
    ...
    ## 执行一个对象的方法时，如果这个对象没有这个方法，则会调用这个对象的 method_missing 方法
  ...
  def test_catching_messages_makes_respond_to_lie
    ...
    assert_nothing_raised do ## 也就是说：调用不存在的方法时，不报错
      catcher.any_method
    end
    assert_equal false, catcher.respond_to?(:any_method) ## 但是使用 respond_to 可以检查出来“是不是真的有”
  end
  ...
  class WellBehavedFooCatcher
    def method_missing(method_name, *args, &block)
      if method_name.to_s[0,3] == "foo"
        ...
        ## 思考：这里的 super 和之前遇到的 super 一样吗？我想，是一样的
      end
    end
  end
